...
# --- Login simples: só registro de e-mail (sem Google Cloud). Bloqueia até preencher.
if not st.session_state.get("user_email") or not str(st.session_state.user_email).strip():
    st.title("WMS Tractian")
    st.markdown("Informe seu e-mail para acessar. As movimentações serão registradas com este e-mail.")
