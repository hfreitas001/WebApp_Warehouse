...
COLUNAS = ["BoxId", "address", "itemCode", "quantity", "uom", "BatchId", "description", "expiryDate"]
...
def add_to_stock(linhas):
    ...
    if "stock" not in st.session_state:
        st.session_state.stock = pd.DataFrame(columns=COLUNAS)
    st.session_state.stock = pd.concat([st.session_state.stock, df], ignore_index=True)


def remove_from_stock(box_ids):
    ids = [box_ids] if isinstance(box_ids, str) else list(box_ids)
    if not ids or "stock" not in st.session_state:
        return
...
def load_data():
    ...
    files = {"items": "Warehouse App (1).xlsx - item_registration.csv", "addr": "Warehouse App (1).xlsx - adress_registration.csv"}
    out = {}
